...
DEFINE_HOOK(0x41EAC0, AITriggerTypeClass_ConditionMet_Gate, 0x5)
{
    ...
    if (!pExt->ExtraPrerequisitesMet(pOwner, pEnemy))
    {
        ...
        AITriggerTypeExt::EmitDebugCancel(pExt, pThis, pOwner, pEnemy);
        return ReturnFalse;
    }

    AITriggerTypeExt::EmitDebugConsider(pExt, pThis, pOwner, pEnemy);
    return 0;
}
...
DEFINE_HOOK(0x6E9443, TeamClass_Update_ScriptSwitch, 0x8)
{
    ...
    AITriggerTypeExt::EvaluateEscort(pTeam);
    AITriggerTypeExt::EvaluateSteamroll(pTeam);
    return 0;
}
