...
// Parse "passing,failing" / "passing" / "failing" / "none" / "never" / "" into a mode struct.
    ...
    AIExtDetailMode& out)
{
    ...
    // Reset to defaults before parsing (so INI value overrides fully).
    ...
    // Simple tokenizer — split on comma
    char buf[128];
    ...
    for (char* tok = strtok_s(buf, ",", &ctx); tok; tok = strtok_s(nullptr, ",", &ctx))
    {
        ...
        if (_stricmp(tok, "passing") == 0) out.show_passing = true;
            ...
            out.show_passing = false;
            out.show_failing = false;
            return;  // short-circuit: never/none overrides everything else
        }
    }
}

// Parse "start,cancel,finish" / subsets into a lifecycle mask struct.
...
void AITriggerTypeExt::ExtData::LoadFromINIFile(CCINIClass* const pINI)
{
    ...
    // Priority 1 debug — Detail sub-fields
